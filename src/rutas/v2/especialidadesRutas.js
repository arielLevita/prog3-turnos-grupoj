...
const validateId = [
    ...
    validarCampos // Usamos el middleware global para no repetir código
];

// Validamos las variables que viajan en la URL (ej: ?limit=10)
...
// Validamos el JSON que manda el usuario en el Body
...
// Middleware que empaqueta las variables de la URL para que el Controlador las reciba ordenadas
const findAllTransformarQueryParams = (req, res, next) => {
    ...
    // Si mandaron "order", vemos si asc es falso para poner DESC, sino por defecto es ASC
    if (order) orderObj[order] = asc !== false ? "ASC" : "DESC";

    req.query.filter = filterObj;
    req.query.order = orderObj;
    next(); // Pasa al siguiente eslabón (el caché o el controlador)
};

// Intercepta el body y lo convierte en nuestro DTO para limpiarlo (trim, toUpperCase)
